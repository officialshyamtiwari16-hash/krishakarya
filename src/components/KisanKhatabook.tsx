          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 sm:p-6 max-w-md w-full shadow-2xl space-y-4 border border-slate-100 dark:border-slate-800 text-slate-900 dark:text-slate-100 my-auto">
            ...
            <form onSubmit={handleSaveEntry} className="space-y-3.5 text-xs">
              ...
              <div className="pt-2 flex items-center gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl text-xs shadow-md cursor-pointer"
                >
                  Save to Khatabook
                </button>
              </div>
          ...
          >
            <div className="flex items-center justify-between border-b dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2 text-rose-600">
                ...
                <h4 className="font-black text-slate-900 dark:text-slate-100 text-sm">Delete Ledger Record</h4>
              </div>
              <button
            ...
            <div className="text-xs text-slate-600 dark:text-slate-300 space-y-2">
              <p>
                Are you sure you want to delete this record: <strong className="text-slate-900 dark:text-slate-100">"{entryToDelete.title}"</strong>?
              </p>
              <div className="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl text-[11px] space-y-1">
                <p><strong>Amount:</strong> ₹{(entryToDelete.amount || 0).toLocaleString('en-IN')}</p>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={() => setEntryToDelete(null)}
                className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-xl cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={() => {
                  onDeleteLedgerEntry(entryToDelete.id);
                  deleteLedgerEntryFromFirestore(entryToDelete.id);
