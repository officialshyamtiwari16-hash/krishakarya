...
export const SahyogiListings: React.FC<SahyogiListingsProps> = ({
  ...
  return (
    <div className="space-y-8">
      {/* Hero Banner */}
      <div className="relative rounded-xl overflow-hidden bg-gradient-to-r from-emerald-900 via-green-800 to-amber-950 p-3 sm:p-4 text-white shadow-sm space-y-2">
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 bg-amber-400/20 text-amber-300 border border-amber-400/40 px-2 py-0.5 rounded-full text-[10px] font-bold">
              <UserCheck className="w-3 h-3 text-amber-300" /> Rated Agricultural Helpers (Sahyogi)
            </div>
          ...
          <button
            ...
            className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-lg text-[11px] shadow-xs transition-all flex items-center gap-1.5 flex-shrink-0 self-start sm:self-center"
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 shadow-sm border border-slate-200/80 dark:border-slate-800 space-y-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar smooth-scroll">
          <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap flex items-center gap-1">
            <Filter className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> Service Type:
          </span>
          {allSkills.map((skill) => (
            <button
              key={skill}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div className="relative md:col-span-2">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400"
            />
            {searchTerm && (
              <button
          ...
          <div>
            <select
              ...
              className="w-full py-2.5 px-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
            >
          ...
          <div className="flex flex-col justify-center px-2">
            <div className="flex justify-between items-center text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1">
              <span>Max Rate: ₹{maxRate}/day</span>
            </div>
            <input
              type="range"
              min="200"
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
          {(searchTerm || selectedSkill !== 'All' || selectedDistrict !== 'All' || maxRate < 1500) ? (
            <button
              ...
              className="text-emerald-700 dark:text-emerald-400 hover:underline font-bold text-[11px] flex items-center gap-1"
            >
          ...
          <div className="text-slate-500 dark:text-slate-400 font-medium">
            Found <span className="font-bold text-emerald-700 dark:text-emerald-400">{filteredSahyogis.length}</span> Sahyogi Profiles
          </div>
        </div>
      </div>
      ...
      {filteredSahyogis.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 text-center space-y-4">
          <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 rounded-full flex items-center justify-center mx-auto">
            <Users className="w-8 h-8" />
          </div>
          <div className="space-y-1">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">No Sahyogi Helpers Registered Yet</h3>
          ...
          <button
            onClick={onOpenAddListing}
            className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs inline-flex items-center gap-2 shadow-sm"
          >
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 px-4 sm:px-5 py-3 border-t border-slate-100 dark:border-slate-800 flex flex-col xs:flex-row items-start xs:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 w-full xs:w-auto justify-end">
                <button
                  ...
                  className="px-3 py-2 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl text-xs border border-slate-200 dark:border-slate-700 transition-colors min-h-[38px]"
                >
                  View Details
                </button>
                <button
                  ...
                  className="px-3.5 py-2 bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl text-xs shadow-sm transition-colors flex items-center gap-1 min-h-[38px]"
                >
      ...
      {selectedSahyogi && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-2 sm:p-4 overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden my-auto max-h-[92vh] flex flex-col text-slate-900 dark:text-slate-100">
